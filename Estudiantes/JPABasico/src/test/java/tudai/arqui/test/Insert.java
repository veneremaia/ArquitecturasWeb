...
public class Insert {
    ...
    public static void main(String[] args) throws IOException {
        ...
        DireccionRepositoryImpl direccionRepository = new DireccionRepositoryImpl(em);
        EstudianteRepositoryImpl estudianteRepository = new EstudianteRepositoryImpl(em);
        CarreraRepositoryImpl carreraRepository = new CarreraRepositoryImpl(em);
        ...
        for (CSVRecord row : parser) {
            direccionRepository.saveDireccion(new Direccion(Integer.parseInt(row.get("id")),row.get("ciudad"), row.get("calle")));

        }
        parser = readFile("carreras");
        for (CSVRecord row : parser) {
            carreraRepository.saveCarrera(new Carrera(Integer.parseInt(row.get("id")),row.get("name")));
        }
        ...
        for (CSVRecord row : parser) {
            ...
            estudianteRepository.saveEstudiante(new Estudiante(Integer.parseInt(row.get("id")),row.get("name"), row.get("lastName"), Integer.parseInt(row.get("age")),
        ...
        for (CSVRecord row : parser) {
            ...
            carreraEstudianteRepository.matricularEstudiante(ce);
        }
